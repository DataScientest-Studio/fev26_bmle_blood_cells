"""
DAG Airflow — Fine-tuning incrémental SANS drift (usage : runs multiples en série)

Identique à blood_cell_incremental_finetune_pipeline mais sans les tâches drift/alerte.
À utiliser quand on enchaîne plusieurs batches manuellement — le drift sera lancé
une seule fois à la fin via blood_cell_drift_monitoring ou manuellement.
...
MAC_MLFLOW_TAILSCALE_URI = f"http://{os.environ['MAC_TAILSCALE_IP']}:5001"
...
dag = DAG(
    dag_id="blood_cell_finetune_only",
    description="Fine-tuning incrémental SANS drift — pour enchaîner plusieurs batches en série",
    schedule_interval=None,  # manuel uniquement
    start_date=datetime(2026, 6, 22),
    catchup=False,
    default_args=default_args,
    tags=["blood-cell", "ml", "incremental", "tiff", "no-drift"],
...
# ── Task 6 : Sync datalake + redémarrage inférence ───────────────────────────
sync = PythonOperator(
...
# ── Task 7 : Fin ──────────────────────────────────────────────────────────────
    ...
    bash_command='echo "Fine-tuning terminé (sans drift)."',
